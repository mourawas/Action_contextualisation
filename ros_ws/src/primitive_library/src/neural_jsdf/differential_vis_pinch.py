...
class differentiate_solver:
    def __init__(self, method="SLSQP", used_link=None, obj_name=None, obj_position=None):
        ...
        sample = [-0.0026752, 0.73955288, 0.1526367, -1.78511253, 0.1432322, -1.04540271,
                  0, 1.31697969, 0.73863557, 0.14285512, 0.263, 0.,
                  0., 0.]
        ...
        self.sphere_collision = o3d.geometry.TriangleMesh.create_sphere(radius=0.033, resolution=30)
        # self.sphere_collision.translate([0.2, 0.2, 0.35])
        self.sphere_collision.translate([0.61, -0.0, 0.133])
        ...
        self.box_1_collision = o3d.geometry.TriangleMesh.create_box(width=0.25,
                                                                    height=0.65,
                                                                    depth=0.1)
        self.box_1_collision.translate([0.55, -0.20, 0.])
        ...
        self.cylinder_collision.translate([0.7, 0.19, 0.18])
        ...
        self.box_collision = o3d.geometry.TriangleMesh.create_box(width=0.25,
                                                                  height=0.25,
                                                                  depth=0.25)
        self.box_collision.translate([0.55, -0.45, 0.])
        ...
        np.random.seed(8)
        env_points = np.concatenate((self.box_1_collision_points,
                                     self.box_collision_points,
    ...
    def motion_planning_vis(self):
        ...
        goal_state = [-0.01575058, 0.76197932, 0.13532024, -2.00838031, 0.18172565,
                      -0.9769772, -0.00924011, 1., 0.2999985, 1.0116041,
                      1.36929732, 0.03641436, 0.2, 0.6]
        ...
        self.vis.add_geometry({'name': 'robot_gaol',
                               'geometry': robot_mesh})
        # self.app.run()
        start_state = [-1.0026752, 1, 0.1526367, -1.78511253, 0.1432322, -1.04540271, 0]
        ...
        state_sequence = [[-1.0026752, 1.0, 0.1526367, -1.78511253, 0.1432322, -1.04540271, 0.0],
                          [-1.0622081120544653, 0.7717783386438439, -0.032859994807873404, -1.7096730677472711,
                           -0.04123912114479207,
                           -1.050613890285485, -0.17382101338921108],
                          [-1.1217410241089305, 0.5435566772876879, -0.21835668961574684, -1.6342336054945423,
                           -0.22571044228958415,
                           -1.0558250705709697, -0.3476420267784221],
                          # [-1.1812739361633957, 0.3153350159315313, -0.4038533844236203, -1.5587941432418135,
                          #  -0.4101817634343761, -1.0610362508564544, -0.5214630401676331],
                          # [-0.6493515006980987, -0.004806582114124321, -0.5607215818904236, -1.630115487400801,
                          #  -0.5334965511363916, -1.0250140263954033, -0.6643587451222798],
                          [-0.49095127052357396, 0.18688989341440676, -0.3867111264178177, -1.7246816930506008,
                           -0.3546910008522937, -1.0130048197965524, -0.5005790863417102],
                          [-0.3325510403490493, 0.37858636894293785, -0.2127006709452119, -1.8192478987004006,
                           -0.1758854505681958, -1.0009956131977016, -0.3367994275611401],
                          [-0.17415081017452466, 0.5702828444714689, -0.03869021547260593, -1.9138141043502004,
                           0.0029200997159021178, -0.9889864065988508, -0.17301976878057002]]
    ...
    def obj_collision_free_constraint(self, robot_q):
        ...
        points = np.concatenate((self.box_1_collision_points, self.cylinder_collision_points))
    ...
    def obj_collision_free_constraint_grad(self, robot_q):
        points = np.concatenate((self.cylinder_collision_points, self.box_1_collision_points))
